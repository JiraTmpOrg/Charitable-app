...
class userDonation() : Fragment() {
    ...
    private fun userDonationsInitialize(){
        newsArrayList= arrayListOf<Item>()
        val db= DatabaseManager.db
        val heading=db.getUserDonation()
        for (i in heading.indices)
        {
            val news= Item(R.drawable.a,heading[i],)
